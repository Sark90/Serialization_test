...
public class RWObj {
    ...
    public void write(boolean toXML, Object...objects) {
        if (toXML) {
            writeXML(objects);
            //writeXMLTest(objects);
        } else {
            writeJSON(objects);
        }
    }

    private void writeXMLTest(Object...objects) {
        if (objects.length == 0) return;
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(XML_FILE))) {
            oos.writeChars("<?xml version=\"1.0\" encoding=\"UTF-8\" ?>");
            oos.writeChars("<!DOCTYPE " + objects[0].getClass().getSimpleName() + "s>");
            oos.writeChars("<" + objects[0].getClass().getSimpleName() + "s>");
            for (Object o : objects) {
                oos.writeChars("\t<" + o.getClass().getSimpleName() + ">");
                Field[] fields = o.getClass().getDeclaredFields();
                for (Field field : fields) {
                    field.setAccessible(true);
                    String fieldName = field.getName();
                    oos.writeChars(XML_PARAM_BEGIN + fieldName + ">" + field.get(o) + "</" + fieldName + ">");
                }
                oos.writeChars(XML_OBJ_END + o.getClass().getSimpleName() + ">");
            }
            oos.writeChars("</" + objects[0].getClass().getSimpleName() + "s>");
